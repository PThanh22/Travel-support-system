...
def About_us():
    st.markdown("# Recommended travel destination 🧳")
    st.sidebar.markdown("# Recommended travel destination 🧳")
...
def Recommend_Destination():
    st.markdown("# Recommend Destination  ✈️")
    st.sidebar.markdown("# Destinatio ✈️")
...
def create_travel_plan(days, with_who, destinations, max_price):
    if len(destinations) == 0:
        st.write("Không có điểm đến nào được chọn!")
        return

    if len(destinations) < days:
        st.write(f"Số ngày {days} lớn hơn số điểm đến {len(destinations)}!")
        return
    ...
    for i in range(days):
        ...
        for destination in destinations:
            if get_rating(with_who, destination) is None:
                continue
            if get_price(destination) < min_price and get_price(destination) <= max_price:
                min_price = get_price(destination)
                best_destination = destination
        if best_destination is None:
            st.write(f"Không tìm thấy điểm đến nào có giá trị phù hợp với yêu cầu!")
            return
        plan.append(best_destination)
        total_cost += min_price

    st.write("**Danh sách kế hoạch du lịch:**")
    for i, destination in enumerate(plan):
        st.write(f"{i+1}. {get_name(destination)} - {get_price(destination)}")
...
page_names_to_funcs = {
    "About_us": About_us,
    "Chatbot": Chatbot,
    "Recommend Hottel": Recommend_Hotel,
    "Recommend Destination": Recommend_Destination,
}
